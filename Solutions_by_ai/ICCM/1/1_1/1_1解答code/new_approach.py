...
def construct_new_matrix():
    ...
    # 科目1：A, B, E, F排名1-4，C, D排名5-6
    subject1 = [1, 2, 5, 6, 3, 4, 7, 8]
    
    # 科目2：C, D, E, F排名1-4，A, B排名5-6
    subject2 = [5, 6, 1, 2, 3, 4, 7, 8]
    
    return [subject0, subject1, subject2]

# 构造一个更复杂的矩阵
def construct_complex_matrix():
    """
    构造一个复杂的排名矩阵，让A-F都有机会成为潜在冠军
    :return: 3x8的排名矩阵
    """
    # 科目0：A-F排名1-6，G-H排名7-8
    subject0 = [1, 2, 3, 4, 5, 6, 7, 8]
    
    # 科目1：A, C, E排名1-3，B, D, F排名4-6
    subject1 = [1, 4, 2, 5, 3, 6, 7, 8]
    
    # 科目2：B, D, F排名1-3，A, C, E排名4-6
    subject2 = [4, 1, 5, 2, 6, 3, 7, 8]
...
if __name__ == "__main__":
    # 测试新的构造方法
    print("测试新的构造方法：")
    new_matrix = construct_new_matrix()
    count, champions, order_champions = count_potential_champions(new_matrix)
    ...
    # 测试复杂矩阵
    print("\n\n测试复杂矩阵：")
    complex_matrix = construct_complex_matrix()
    count, champions, order_champions = count_potential_champions(complex_matrix)
    print(f"潜在冠军数量: {count}")
    print(f"潜在冠军学生: {champions}")
